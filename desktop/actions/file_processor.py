...
def _get_api_key() -> str:
    ...
    try:
        k = json.loads(config_path.read_text(encoding="utf-8")).get("gemini_api_key", "")
        return k if k not in _PLACEHOLDERS else ""
    except Exception:
        return ""
...
def _llm_describe(prompt: str, context: str = "") -> str:
    """LLM fallback for when Gemini is unavailable — uses text_llm routing."""
    try:
        import sys, pathlib
        sys.path.insert(0, str(pathlib.Path(__file__).resolve().parent.parent))
        from core.text_llm import complete
        full = f"{prompt}\n\n{context}" if context else prompt
        return complete(full, max_tokens=512, timeout=60)
    ...
    if action in ("describe", "ocr", "analyze", "read", "extract_text"):
        try:
            ...
            if model:
                ...
                # No Gemini key — describe via text LLM (no vision, give file info)
                result = _llm_describe(
                    f"{prompt}\n\nNote: No vision API available. File: {path.name}, "
                    f"size: {path.stat().st_size//1024}KB, format: {img.format}, "
                    f"dimensions: {img.width}x{img.height}px, mode: {img.mode}"
                )
